...
**注意: 同じテーブルを、複数のカタログの「正」にしてはいけません。** 「最新の場所」を持つ1行が2か所にあると、片方で書き込んだ結果がもう片方に反映されず、2つの「最新」が食い違います。読み取りだけを別のカタログに見せる、といった使い方はありますが、書き込みの窓口は1つのカタログに絞ります。（この食い違いを扱うのが、いわゆるカタログ間の相互運用の課題です）
