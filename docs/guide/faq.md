...
### マネージドのカタログ（Glueなど）と、自前で立てるPolarisは何が違うの？

**どちらも「最新のmetadata.jsonの場所を答えるサーバー」がいる点は同じで、違うのは「そのサーバーを誰が動かすか」です。**

| | マネージド（Glue、Snowflake Open Catalogなど） | 自前（Polarisを自分で立てる） |
|---|---|---|
| サーバーは誰が動かす？ | クラウド／ベンダー（裏側は見えない） | 自分（K8sのDeploymentなど） |
| 「最新の場所」の保存先 | ベンダーが管理（見えない） | 自分で選ぶ（DBやメモリ） |
| 運用の手間 | ほぼ無い | 起動、更新、障害対応、バックアップが自分の責任 |
| 自由度 | 提供された機能の範囲 | 設定や権限の作り込みが自由 |

つまり、マネージドを使うときも、裏では同じように「場所を答えるサーバー」が動いています。それを見せずに、代わりに運用してくれているだけです。（Snowflake Open Catalogは、Polarisをベンダーが動かしてくれるサービスです）

#### 筆者がPolarisを自前で立てた理由

筆者のプロジェクト（azure-realtime-lakehouse）では、AzureにGlueのような、そのまま使えるIceberg用のマネージドカタログが無かったことに加えて、**「なるべくマネージドに頼らず、Azure×Kubernetesで一から組むと何が起きるか」を、意図的に試す**目的でPolarisを自前で立てました。

その結果、マネージドなら見えない問題を自分で解くことになりました。例えば、Polaris自身がストレージに触れる権限を持っていなかったことが原因で、テーブル作成が失敗しました。（筆者が実機で確認）

また、検証用の構成なので、Polarisの「最新の場所」の保存先はメモリ（再起動すると消える）にしていました。本番で自前運用するなら、ここをDBなどの永続的な保存先にする必要があります。マネージドなら、この心配自体がありません。
